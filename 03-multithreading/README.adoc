...
[mermaid, target=gil-concept, format=svg]
....
graph TD
...
[mermaid, target=io-bound-threading, format=svg]
....
graph LR
...
[mermaid, target=cpu-bound-threading, format=svg]
....
graph TD
...
[mermaid, target=thread-lifecycle, format=svg]
....
stateDiagram-v2
    [*] --> Created: Thread()
    Created --> Ready: start()
...
|Thread creation
|~100-500 μs
|OS thread + Python structures

|Context switch
|~1-10 μs
|Depends on CPU and OS
...
link:../04-multiprocessing/README.adoc[Module 4: Multiprocessing] - True parallelism with multiple Python processes
